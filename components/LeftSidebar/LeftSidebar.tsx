...
import { HiOutlineSearch, HiOutlineBell, HiOutlineMail, HiHome } from "react-icons/hi";
...
export default function LeftSidebar({ avatarUrl }: LeftSidebarProps) {
    ...
    <div className={styles.leftSidebar}>
      <div>
        ...
        <button className={styles.postButton}>
          <svg viewBox="0 0 24 24" aria-hidden="true" className={styles.featherIcon} fill="white" >
            <g>
        ...
        <div className={styles.miniProfile} onClick={toggleMenu}>
          ...
          {isMenuOpen && (
            <div className={styles.profileMenu}>
              <div className={styles.profileMenuItem} onClick={handleSignOut}>Log out @{user.displayName || "guest"}</div>
            </div>
          )}
        </div>
      ) : (
